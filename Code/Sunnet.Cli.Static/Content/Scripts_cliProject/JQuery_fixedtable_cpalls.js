(function ($) {
    jQuery.fn.fixedTable = function (options) {
        ...
        return this.each(function () {
            ...
            $("#" + tableID + "_tableHead").css({ "overflow": "hidden", "position": "relative", "z-index": "45", "background-color": fixColumnColor });
            $("#" + tableID + "_tableColumn").css({ "overflow": "hidden", "position": "relative", "z-index": "40", "background-color": fixColumnColor });
            $("#" + tableID + "_tableData").css({ "overflow": "scroll", "max-width": maxwidth, "position": "relative", "z-index": "35" });

            var scrollbarWidth = $("#" + tableID + "_tableData")[0].offsetWidth - $("#" + tableID + "_tableData")[0].clientWidth;//获取滚动条宽度
            if (scrollbarWidth == 0) {
                scrollbarWidth = 17;
            }
            var scrollbarWidthDouble = scrollbarWidth * 2;

            if (oldtable.width() <= maxwidth) {   //如果table本身宽度小于最大宽度值，则将宽度设置为原始数据宽度
                $("#" + tableID + "_tableHead").css("width", oldtable.width() - scrollbarWidthDouble);//去除滚动条的宽度
                $("#" + tableID + "_tableData").css("width", oldtable.width() - scrollbarWidth);
            }
            else {
                $("#" + tableID + "_tableHead").css("max-width", parseInt(maxwidth - scrollbarWidth));
            }
            ...
            if (oldtable.height() <= maxheight) {   //如果最大高度大于数据高度，则将高度设置为自定义高度+滚动条高度（去除多余部分）
                ...
                $("#" + tableID + "_tableLayout").css("height", customHeight + scrollbarWidth);
            }
            else {
                $("#" + tableID + "_tableColumn").css("max-height", parseInt(maxheight - scrollbarWidthDouble));
                $("#" + tableID + "_tableData").css("max-height", parseInt(maxheight - scrollbarWidth));
            }
