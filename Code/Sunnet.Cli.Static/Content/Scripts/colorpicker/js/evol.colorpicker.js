...
(function ($, undefined) {
    ...
    $.widget("evol.colorpicker", {
        ...
        _paletteHTML: function () {
            ...
            var h = '<div class="Lee evo-pop' + _ie + ' ui-widget ui-widget-content ui-corner-all"' +
                (this._isPopup ? ' style="position:absolute;box-sizing:content-box;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;"' : '') + '>' +
                // palette
        ...
        _bindColors: function () {
            ...
            this._palette
                .on('click', sel, function (evt) {
                    if (that._enabled) {
                        var $this = $(this);
                        if (that.options.warning && $(that.options.warningObjId).css("display") != "none") {
                            $.when(waitingConfirm("Selecting a background color fill will clear out the background image you have selected. Would you still like to proceed?", "Ok", "Cancel"))
                                .done(function () {
                                    that._setValue($this.hasClass('evo-transparent') ? transColor : toHex3($this.attr('style').substring(17)));
                                    that._active = false;
                                    if (that.options.customOperation && typeof (eval("" + that.options.customOperation + "")) == "function")
                                        eval("" + that.options.customOperation + "()");
                                });
                        }
                        else {
                            that._setValue($this.hasClass('evo-transparent') ? transColor : toHex3($this.attr('style').substring(17)));
