...
function require_login($courseorid = null, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    if(!empty($_SESSION["idnumber"])){
        $url=$CFG->sso.'/home/MoodleGlobalCookie?googleid='.$_SESSION["idnumber"].'&userid='.$_SESSION["userid"];
        $clicookievalue=file_get_contents($url);
        setcookie($CFG->sessioncookie, $clicookievalue,0, $CFG->sessioncookiepath,
                $CFG->cookiedomain, $CFG->cookiesecure, $CFG->cookiehttponly);
    }
    ...
    if (!isloggedin()) {
        if ($autologinguest and !empty($CFG->guestloginbutton) and !empty($CFG->autologinguests)) {
            ...
            //redirect(get_login_url());
            //redirect($CFG->cli);
            //$SESSION->wantsurl='http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
            redirect($CFG->cli.'/Home/MoodlePerPageAutoLogin');
            //redirect($CFG->cli.'/Home/MoodlePerPageAutoLogin?returnurl='.'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
            exit; // Never reached.
        }
    }
    ...
    //if(empty($token)){
    //    redirect($CFG->cli.'/Home/MoodleGlobalCookie?returnurl='.'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
    //}
