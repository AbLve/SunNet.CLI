...
namespace Sunnet.Cli.Practice.Areas.Report.Controllers
{
    public class StudentController : BaseController
    {
        ...
        private void SavePdf(byte[] content, string fileName, AssessmentLanguage language, PdfType type = PdfType.Assessment_Landscape, int pdfPageWidth = 0)
        {

            PdfProvider pdfProvider = new PdfProvider(type, "", "", "", "#reportHtml", pdfPageWidth);
            pdfProvider.SavePdf(content, fileName);
        }
        ...
        #region Student summary,percentile rank,parent reports
        ...
        public ActionResult StudentReportWaves(int assessmentId, int year, int id)
        {
            ...
            var student = _practiceBusiness.GetStudent(id);
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult StudentReports(int assessmentId, StudentAssessmentLanguage language, int year, int id,
            string waves, string measures, string reportType, bool entireClass, bool printComments, string parentReportType,
            DateTime? dobStartDate, DateTime? dobEndDate, bool export = true)
        {
            ...
            try
            {
                ...
                int fromMonths = 0;
                int fromDays = 0;
                if (dobEndDate != null && entireClass)
                {
                    CommonAgent.CalculatingAge(time.Year, dobEndDate.Value, out fromMonths, out fromDays);
                }

                int toMonths = int.MaxValue;
                int toDays = 0;
                if (dobStartDate != null && entireClass)
                {
                    CommonAgent.CalculatingAge(time.Year, dobStartDate.Value, out toMonths, out toDays);
                }
                ...
                {
                    StudentSummaryReport(assessmentId, language, year, id, waveMeasures, entireClass, printComments, fromMonths, toMonths,
                       export);
                }
                else if (reportType.ToLower() == "percentilerank")
                {
                    StudentPercentileRankReport(assessmentId, language, year, id, waveMeasures, entireClass, printComments, fromMonths, toMonths,
                       export);
                }
                else if (reportType.ToLower() == "parent")
                {
                    id = entireClass ? 0 : id;
                    StudentParentReport(assessmentId, language, year, id, waveMeasures, parentReportType, fromMonths, toMonths, export);
                }
                return null;
            }
            catch (Exception ex)
        ...
        private ActionResult StudentSummaryReport(int assessmentId, StudentAssessmentLanguage language, int year, int id,
            Dictionary<Wave, IEnumerable<int>> waveMeasures,
            bool entireClass, bool printComments, int fromMonths, int toMonths, bool export = true)
        {
            ...
            id = entireClass ? 0 : id;
            Dictionary<object, List<ReportRowModel>> datas = _practiceBusiness.GetStudentReport(assessmentId, language,
                printComments, UserInfo, year, id, waveMeasures, fromMonths, toMonths);
        ...
        private ActionResult StudentPercentileRankReport(int assessmentId, StudentAssessmentLanguage language, int year, int id,
            Dictionary<Wave, IEnumerable<int>> waveMeasures,
            bool entireClass, bool printComments, int fromMonths, int toMonths, bool export = true)
        {
            ...
            id = entireClass ? 0 : id;
            Dictionary<object, List<ReportRowModel>> datas = _practiceBusiness.GetStudentPercentileRankReport(assessmentId, language,
                printComments, UserInfo, year, id, waveMeasures, fromMonths, toMonths);
            ViewBag.Datas = datas.ToDictionary(x => (int)x.Key, x => x.Value);
            ViewBag.Pdf = export;
            if (export)
        ...
        [HttpPost]
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        private string StudentParentReport(int assessmentId, StudentAssessmentLanguage language, int year, int studentId,
               Dictionary<Wave, IEnumerable<int>> waveMeasures, string parentReportType, int fromMonths, int toMonths, bool export = true)
        {
            ...
            if (studentId == 0)
            {
                studentIds = _practiceBusiness.GetStudentIdByCondition(x =>
            ...
            foreach (var id in studentIds)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            Dictionary<object, List<ReportRowModel>> datas = _practiceBusiness.GetStudentParentReport(assessmentId, language, false,
                            UserInfo, year, id, waveMeasures);
                            ...
                            contentPage = contentPage.Replace("{StudentName}", student.StudentName);
                            ...
                            foreach (var reportValue in reportValues)
                            {
                                ...
                                foreach (var rowModel in reportValue.Value)
                                {
                                    ...
                                    if (measureModel != null && measureModel.Links != null && measureModel.Links.Count > 0)
                                    {
                                        ...
                                        {
                                            strLink += "<a href='" + lnk.Link + "'>" + lnk.DisplayText + "</a><br />";
                                        }
                                    }
                                        ...
                                        if (parentMeasureModel != null && parentMeasureModel.Links != null && parentMeasureModel.Links.Count > 0)
                                        {
                                            foreach (var lnk in parentMeasureModel.Links)
                                            {
                                                if (lnk.LinkType == 1 && lnk.Status == EntityStatus.Active)
                                                    strParentLink += "<a href='" + lnk.Link + "'>" + lnk.DisplayText + "</a><br />";
                                            }
                                        }
                        ...
                        if (parentReportType.ToLower() == "pin")
                        {
                            ...
                            var pinContent = _practiceBusiness.GeneratePDFForParentReport(student);
                            SavePdf(pinContent, pinName, assessment.Language, PdfType.ParentPinPage);
                            var pinItem = new ParentReportEntity { ReportName = pinDisplayName, StudentId = id };
                            reports.Add(pinItem);
                        }
                        else if (parentReportType.ToLower() == "onlyreport")
                        {
                            ...
                            var contentDisplayName = "PinReport_content_" + Guid.NewGuid().ToString().Replace("-", "");
                            var contentName = CreatePinReportTempName(id, contentDisplayName);
                            //var pinContent = _studentBusiness.GeneratePDFForParentReport(student);
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult Summary(int assessmentId, int year, bool export = true)
        {
            List<MeasureHeaderModel> measures;
            List<MeasureHeaderModel> parentMeasures;
            _cpallsBusiness.BuilderHeader(assessmentId, year, Wave.BOY, out measures, out parentMeasures, true);
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult ClassPercentileRank(int assessmentId, int year, bool export = true)
        {
            List<MeasureHeaderModel> measures;
            List<MeasureHeaderModel> parentMeasures;
            _cpallsBusiness.BuilderHeader(assessmentId, year, Wave.BOY, out measures, out parentMeasures, true);
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult PercentageofSatisfactory(int assessmentId, StudentAssessmentLanguage language, int year, string waves,
            DateTime? startDate, DateTime? endDate, DateTime? dobStartDate, DateTime? dobEndDate, bool export = false)
        {
            ...
            Dictionary<object, List<ReportRowModel>> reportsForChart = _practiceBusiness.GetStudentSatisfactoryReport(
                assessmentId, language, UserInfo, year,
                waveMeasures, startDate ?? StartDate, (endDate ?? EndDate).AddDays(1).Date, benchmarks, false, fromMonths, toMonths);
            Dictionary<object, List<ReportRowModel>> reportsForTable = _practiceBusiness.GetStudentSatisfactoryReport(
                assessmentId, language, UserInfo, year,
                waveMeasures, startDate ?? StartDate, (endDate ?? EndDate).AddDays(1).Date, benchmarks, true, fromMonths, toMonths);
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult PercentageofSatisfactory_Pdf(int assessmentId, StudentAssessmentLanguage language, int year, string waves,
              List<string> imgSources, DateTime? startDate, DateTime? endDate, DateTime? dobStartDate, DateTime? dobEndDate, bool export = false)
        {
            ...
            var benchmarks = _adeBusiness.GetIEnumBenchmarks(assessmentId);
            var waveMeasures = JsonHelper.DeserializeObject<Dictionary<Wave, IEnumerable<int>>>(waves);
            Dictionary<object, List<ReportRowModel>> reportsForChart = _practiceBusiness.GetStudentSatisfactoryReport(assessmentId, language, UserInfo, year,
                waveMeasures, startDate ?? StartDate, (endDate ?? EndDate).AddDays(1).Date, benchmarks, false, fromMonths, toMonths);
            Dictionary<object, List<ReportRowModel>> reportsForTable = _practiceBusiness.GetStudentSatisfactoryReport(assessmentId, language, UserInfo, year,
                waveMeasures, startDate ?? StartDate, (endDate ?? EndDate).AddDays(1).Date, benchmarks, true, fromMonths, toMonths);
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult SummaryWithAverge(int assessmentId, StudentAssessmentLanguage language, int year, string waves,
            ...
            int sortMeasureId = 0, string orderDirection = "ASC", string sortMeasureName = "", bool export = false)
        {
            ...
            var waveMeasures = JsonHelper.DeserializeObject<Dictionary<Wave, IEnumerable<int>>>(waves);
            Dictionary<object, List<ReportRowModel>> reports = _practiceBusiness.GetClassSummaryReport(assessmentId, language, year, waveMeasures, startDate ?? StartDate, (endDate ?? EndDate).AddDays(1).Date, fromMonths, toMonths, UserInfo);
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult SummaryPercentileRankAveragePdf(int assessmentId, StudentAssessmentLanguage language, int year, string waves,
            DateTime? startDate, DateTime? endDate, DateTime? dobStartDate, DateTime? dobEndDate,
            int sortMeasureId = 0, string orderDirection = "ASC", string sortMeasureName = "", bool export = false)
        {
            var assessment = _adeBusiness.GetBaseAssessmentModel(assessmentId);
            ...
            var waveMeasures = JsonHelper.DeserializeObject<Dictionary<Wave, IEnumerable<int>>>(waves);
            Dictionary<object, List<ReportRowModel>> reports = _practiceBusiness.GetClassPercentileRankReport(assessmentId, language, UserInfo, year, waveMeasures,
                startDate ?? StartDate, (endDate ?? EndDate).AddDays(1).Date, fromMonths, toMonths);
        ...
        #region Growth
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult Growth(int assessmentId, int year)
        {
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult Growth_Pdf(int assessmentId, StudentAssessmentLanguage language, int year,
            GrowthReportType type, string waves, string measures, DateTime? startDate, DateTime? endDate, DateTime? dobStartDate, DateTime? dobEndDate)
        {
            var assessment = _adeBusiness.GetBaseAssessmentModel(assessmentId);
            if (assessment == null) return View("NoData");
            if (assessment.TheOtherId > 0 && (byte)assessment.TheOtherLang == (byte)language)
            ...
            var reports = _practiceBusiness.GetClassGrowthPdf(assessmentId, language, UserInfo, year, type, waveMeasures,
                startDate ?? StartDate, (endDate ?? EndDate).AddDays(1).Date, benchmarks, fromMonths, toMonths);
        ...
        public ActionResult Growth_Pdf_Download(int assessmentId, StudentAssessmentLanguage language, GrowthReportType type,
            int year, string waves, string measures, List<string> imgSources, DateTime? startDate, DateTime? endDate,
            DateTime? dobStartDate, DateTime? dobEndDate, bool export)
        {
            var assessment = _adeBusiness.GetBaseAssessmentModel(assessmentId);
            if (assessment == null) return View("NoData");
            ViewBag.AssessmentName = assessment.Name;
            ...
            reports = reports ??
                      _practiceBusiness.GetClassGrowthPdf(assessmentId, language, UserInfo, year, type, waveMeasures,
                          startDate ?? StartDate, (endDate ?? EndDate).AddDays(1).Date, benchmarks, fromMonths, toMonths);
            if (reports == null || !reports.Any())
            {
                return View("NoData");
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult Completion(int assessmentId, int year)
        {
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult CompletionPdf(int assessmentId, int year, string waves,
            StudentAssessmentLanguage language, DateTime? startDate, DateTime? endDate, DateTime? dobStartDate, DateTime? dobEndDate)
        {
            ...
            CompletionReportList report = new CompletionReportList();

            foreach (var v in waveMeasures)
            {
                if (v.Value.Any())
                {
                    report = _practiceBusiness.GetReport_Class(UserInfo.ID, assessmentId, year.ToSchoolYearString(),
                  v.Value.ToList(), v.Key, language, startDate ?? StartDate, (endDate ?? EndDate).AddDays(1).Date, fromMonths, toMonths, ref teacherName);
                    break;
                }
            }
            ViewBag.UserName = UserInfo.FirstName + " " + UserInfo.LastName;
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.AssessmentPracticeArea, PageId = PagesModel.Practice, Anonymity = Anonymous.Verified, Parameter = "assessmentId")]
        public ActionResult CompletionPdf_Export(int assessmentId, int year, string waves,
            StudentAssessmentLanguage language, string imgSource, string imgSourcePercent,
            DateTime? startDate, DateTime? endDate, DateTime? dobStartDate, DateTime? dobEndDate, bool export = true)
        {
            var assessment = _adeBusiness.GetBaseAssessmentModel(assessmentId);
            if (assessment == null) return View("NoData");
            ViewBag.AssessmentName = assessment.Name;
        ...
        private int GetNum(CompletionReportList report)
        {
            if (report == null)
                return 0;
