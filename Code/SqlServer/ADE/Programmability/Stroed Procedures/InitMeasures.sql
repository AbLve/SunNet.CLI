...
/****** Object:  StoredProcedure [dbo].[InitMeasures]    Script Date: 6/29/2015 14:35:24 ******/
IF EXISTS(SELECT 1 FROM sys.objects WHERE object_id = object_id('InitMeasures'))
...
BEGIN
	...
	BEGIN TRY
	  BEGIN TRANSACTION;
		...
		Benchmark = CutOffScore,Goal = -1,TotalScored,Comment = ''  
		...
		INSERT INTO StudentMeasures 
		SELECT DISTINCT SAId = @StudentAssessmentId,MeasureId,[Status] = 1, PauseTime = 0,CreatedOn=GETDATE(),UpdatedOn = GETDATE(),TotalScore = ISNULL(TotalScore,0),
		Benchmark = -1,Goal = -1,TotalScored,Comment = ''
		FROM #HasCutoffScore HCS
