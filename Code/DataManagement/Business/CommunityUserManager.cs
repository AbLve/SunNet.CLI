...
{
    public class CommunityUserManager : Base
    {
        ISunnetLog _log;
        ...
        public bool PendingCommunityUser()
        {
            var pendingList = _bupTaskBusiness.GetList(BUPType.CommunityUser).Where(c => c.Status == BUPStatus.Pending);
            foreach (var pending in pendingList)
            {
                PendingCommunityUserByOne(pending);
            }

            return false;
        }

        private bool PendingCommunityUserByOne(BUPTaskEntity groupModel)
        {
