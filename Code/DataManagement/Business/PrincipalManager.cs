...
{
    public class PrincipalManager : Base
    {
        ISunnetLog _log;
        ...
        public bool PendingPrincipal()
        {
            var pendingList = _bupTaskBusiness.GetList(BUPType.Principal).Where(c => c.Status == BUPStatus.Pending);
            foreach (var pending in pendingList)
            {
                PendingPrincipalByOne(pending);
            }

            return false;
        }

        private bool PendingPrincipalByOne(BUPTaskEntity groupModel)
        {
