...
namespace Sunnet.Cli.MainSite.Areas.BUP.Controllers
{
    public class ClassroomController : BaseController
    {
        ...
        // GET: /BUP/Classroom/
        [CLIUrlAuthorizeAttribute(Account = Authority.Index, PageId = PagesModel.BUP_Classroom, Anonymity = Anonymous.Verified)]
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.Index, PageId = PagesModel.BUP_Classroom, Anonymity = Anonymous.Verified)]
        public ActionResult DataProcess(int communityId)
        {
            ...
            errorMsg = _bupProcessBusiness.InvalidateFile(uploadPath, 9, BUPType.Classroom, out dt);
            if (!string.IsNullOrEmpty(errorMsg))
            {
                ModelState.AddModelError("", errorMsg);
                return View("index");
            ...
            int identity = 0;
            errorMsg = _bupProcessBusiness.ProcessClassroom(dt, UserInfo.ID, originFileName, virtualPath, UserInfo, communityId, out identity);
            if (!string.IsNullOrEmpty(errorMsg))
            {
                ModelState.AddModelError("", errorMsg);
                return View("index");
        ...
        {
            int total = 0;
            List<BUPTaskEntity> list = _bupTaskBusiness.GetBupTasks(BUPType.Classroom, UserInfo, sort, order, first, count, out total);
        ...
        public string Delete(int ID)
        {
            ...
            sql.AppendFormat("delete BUP_Tasks where ID = {0};", ID)
                .AppendFormat(" delete BUP_Classrooms where TaskId = {0};", ID);
