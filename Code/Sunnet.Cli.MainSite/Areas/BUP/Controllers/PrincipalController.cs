...
namespace Sunnet.Cli.MainSite.Areas.BUP.Controllers
{
    public class PrincipalController : BaseController
    {
        ...
        // GET: /BUP/Principal/
        [CLIUrlAuthorizeAttribute(Account = Authority.Index, PageId = PagesModel.BUP_Principal, Anonymity = Anonymous.Verified)]
        ...
        public ActionResult DataProcess(bool isSpecialist, string invitation, int communityId)
        {
            ...
            if (isSpecialist)
            {
                errorMsg = _bupProcessBusiness.InvalidateFile(uploadPath, 15, BUPType.SchoolSpecialist, out dt);
                if (!string.IsNullOrEmpty(errorMsg))
                {
                    ModelState.AddModelError("", errorMsg);
                    return View("index");
            ...
            {
                errorMsg = _bupProcessBusiness.InvalidateFile(uploadPath, 15, BUPType.Principal, out dt);
                if (!string.IsNullOrEmpty(errorMsg))
                {
                    ModelState.AddModelError("", errorMsg);
                    return View("index");
            ...
            int identity = 0;
            errorMsg = _bupProcessBusiness.ProcessPrincipal(dt, UserInfo.ID, isSpecialist,
                originFileName, virtualPath, invitation, UserInfo, communityId, out identity);
            if (!string.IsNullOrEmpty(errorMsg))
            {
                ModelState.AddModelError("", errorMsg);
                return View("index");
        ...
        public string BUPData(bool isSpecialist, string sort, string order, int first, int count)
        {
            ...
            List<BUPTaskEntity> list = _bupTaskBusiness.GetBupTasks(
                isSpecialist ? BUPType.SchoolSpecialist : BUPType.Principal, UserInfo, sort, order, first, count, out total);
        ...
        public string Delete(int ID)
        {
            ...
            sql.AppendFormat("delete BUP_Tasks where ID = {0};", ID)
                .AppendFormat(" delete BUP_Principals where TaskId = {0};", ID);
