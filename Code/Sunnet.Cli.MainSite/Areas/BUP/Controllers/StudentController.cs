...
namespace Sunnet.Cli.MainSite.Areas.BUP.Controllers
{
    public class StudentController : BaseController
    {
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.Index, PageId = PagesModel.BUP_Student, Anonymity = Anonymous.Verified)]
        ...
        public ActionResult DataProcess()
        {
            ...
            errorMsg = _bupProcessBusiness.InvalidateFile(uploadPath, 20, BUPType.Student, out dt);
            if (!string.IsNullOrEmpty(errorMsg))
            {
                ModelState.AddModelError("", errorMsg);
                return View("index");
            ...
            int identity = 0;
            errorMsg = _bupProcessBusiness.ProcessStudent(dt, UserInfo.ID, originFileName, virtualPath, UserInfo, out identity);
            if (!string.IsNullOrEmpty(errorMsg))
            {
                ModelState.AddModelError("", errorMsg);
                return View("index");
        ...
        {
            int total = 0;
            List<BUPTaskEntity> list = _bupTaskBusiness.GetBupTasks(BUPType.Student, UserInfo, sort, order, first, count, out total);
        ...
        public string Delete(int ID)
        {
            ...
            sql.AppendFormat("delete BUP_Tasks where ID = {0};", ID)
                .AppendFormat(" delete BUP_Students where TaskId = {0};", ID);
