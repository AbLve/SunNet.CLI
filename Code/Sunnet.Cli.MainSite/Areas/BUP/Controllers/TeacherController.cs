...
namespace Sunnet.Cli.MainSite.Areas.BUP.Controllers
{
    public class TeacherController : BaseController
    {
        ...
        // GET: /BUP/Teacher/
        [CLIUrlAuthorizeAttribute(Account = Authority.Index, PageId = PagesModel.BUP_Teacher, Anonymity = Anonymous.Verified)]
        ...
        public ActionResult DataProcess(string invitation)
        {
            ...
            errorMsg = _bupProcessBusiness.InvalidateFile(uploadPath, 19, BUPType.Teacher, out dt);
            if (!string.IsNullOrEmpty(errorMsg))
            {
                ModelState.AddModelError("", errorMsg);
                return View("index");
            ...
            int identity = 0;
            errorMsg = _bupProcessBusiness.ProcessTeacher(dt, UserInfo.ID, invitation,
                originFileName, virtualPath, UserInfo, out identity);
            if (!string.IsNullOrEmpty(errorMsg))
            {
                ModelState.AddModelError("", errorMsg);
                return View("index");
        ...
        {
            int total = 0;
            List<BUPTaskEntity> list = _bupTaskBusiness.GetBupTasks(BUPType.Teacher, UserInfo, sort, order, first, count, out total);
        ...
        public string Delete(int ID)
        {
            ...
            sql.AppendFormat("delete BUP_Tasks where ID = {0};", ID)
                .AppendFormat(" delete BUP_Teachers where TaskId = {0};", ID);
