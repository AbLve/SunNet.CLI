...
{
    public class SchoolSpecialistController : BaseController
    {
        ...
        public SchoolSpecialistController()
        {
        ...
        //
        // GET: /Invitation/SchoolSpecialist/
        [CLIUrlAuthorizeAttribute(Account = Authority.Index, PageId = PagesModel.School_Specialist, Anonymity = Anonymous.Verified)]
        public ActionResult Index()
        {
            InitAccessOperation();
            ViewBag.RoleType = Role.School_Specialist;
        ...
        {
            InitAccessOperation();
            ViewBag.RoleType = Role.School_Specialist_Delegate;
            return View();
        }
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.Add, PageId = PagesModel.School_Specialist, Anonymity = Anonymous.Verified)]
        public ActionResult New(int roleType)
        {
            ...
            if ((Role)roleType == Role.School_Specialist_Delegate)
                ViewBag.ParentSchoolNames = UserInfo.UserCommunitySchools.Select(x => x.School.Name).ToArray();
            return View(principalEntity);
        }

        [CLIUrlAuthorizeAttribute(Account = Authority.Edit, PageId = PagesModel.School_Specialist, Anonymity = Anonymous.Verified)]
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.View, PageId = PagesModel.School_Specialist, Anonymity = Anonymous.Verified)]
            ...
            bool? isInvite, int[] chkPackages, int schoolId, bool confirm = false)
        {
            ...
            if (principal.UserInfo.Role == Role.School_Specialist_Delegate)
                schoolId = 0;
            ...
            if (principal.ID == 0)
            {
                ...
                if (principal.UserInfo.Role == Role.School_Specialist)
                {
                    principal.UserInfo.Role = Role.School_Specialist;
        ...
        private bool CheckExisted(PrincipalEntity principal, int schoolId, PostFormResponse response)
        {
            ...
            {
                if (existedUserRole == Role.School_Specialist)
                {
                    ...
                    {
                        // 已存在School Specialist 角色的并且FirstName,LastName,Email相同的记录,但是操作者没有权限(意外勾选了不属于操作者的权限)
                        response.Success = true;
        ...
        public string SendEmail(int schoolId, string txtSchool, string email, int userId)
        {
            ...
            {
                RoleType = Role.School_Specialist,
        ...
        public void InternalAccess()
        {
            switch (UserInfo.Role)
            {
                ...
                case Role.School_Specialist_Delegate:
                    ViewBag.SchoolSpecialistNote = "X";
                    break;
                case Role.Community:
        ...
        private void InitAccessOperation()
        {
            ...
            if (UserInfo != null)
            {
                UserAuthorityModel userAuthority = new PermissionBusiness().GetUserAuthority(UserInfo, (int)PagesModel.School_Specialist);
                ...
                if (UserInfo.Role == Role.Super_admin || UserInfo.Role == Role.Principal
        ...
        #region BES
        [CLIUrlAuthorizeAttribute(Account = Authority.Bes, PageId = PagesModel.School_Specialist, Anonymity = Anonymous.Verified)]
        public ActionResult SchoolSpecialistBES()
        {
            InitAccessOperation();
            ViewBag.StatusJson = JsonHelper.SerializeObject(EntityStatus.Active.ToList());
            ViewBag.PositionOptions = JsonHelper.SerializeObject(userBusiness.GetPositions((int)Role.Principal).
              ToSelectList(ViewTextHelper.DefaultPleaseSelectText, ""));
            ...
            SchoolSpecialistUserModel userModel = new SchoolSpecialistUserModel();
        ...
        [CLIUrlAuthorizeAttribute(Account = Authority.Bes, PageId = PagesModel.School_Specialist, Anonymity = Anonymous.Verified)]
        public string BESSearch(string txtCommunity, string txtSchool, string specialistCode, string firstName, string lastName, int communityId = 0,
            int schoolId = 0, int status = -1, string sort = "LastName", string order = "Asc", int first = 0, int count = 10)
        {
            var total = 0;
            var expression = PredicateHelper.True<PrincipalEntity>();
            ...
            if (status > 0)
                expression = expression.And(r => (int)r.UserInfo.Status == status);
            if (specialistCode.Trim() != string.Empty)
                expression = expression.And(r => r.PrincipalId.Contains(specialistCode));
            if (firstName.Trim() != string.Empty)
                expression = expression.And(r => r.UserInfo.FirstName.Contains(firstName));
            if (lastName.Trim() != string.Empty)
                expression = expression.And(r => r.UserInfo.LastName.Contains(lastName));
            if (sort == "SchoolName") sort = "LastName";
            var list = userBusiness.SearchPrincipals(UserInfo, Role.School_Specialist, expression, sort, order, first, count, out total, true);
            var result = new { total = total, data = list };
            return JsonHelper.SerializeObject(result);
        }

        [CLIUrlAuthorizeAttribute(Account = Authority.Bes, PageId = PagesModel.School_Specialist, Anonymity = Anonymous.Verified)]
        public string BESSave(string schoolSpecialists, bool confirm = false)
        {
            var response = new PostFormResponse();
            OperationResult res = new OperationResult(OperationResultType.Success);
            List<SchoolSpecialistUserModel> listSchoolSpecialist = JsonHelper.DeserializeObject<List<SchoolSpecialistUserModel>>(schoolSpecialists);
            if (!confirm)
            {
                if (CheckUsersList(listSchoolSpecialist, response))
                    return JsonHelper.SerializeObject(response);
            }
            foreach (SchoolSpecialistUserModel model in listSchoolSpecialist)
            {
                ...
                if (model.ID <= 0)
                {
                    ...
                    item.UserInfo.Role = Role.School_Specialist;
                    item.PrincipalId = userBusiness.SchoolSpecialistCode();
                }
                else
                {
                    item = userBusiness.GetPrincipal(model.ID, UserInfo, Role.School_Specialist);
                }
        ...
        private bool CheckUsersList(List<SchoolSpecialistUserModel> listPrincipal, PostFormResponse response)
        {
            ...
            foreach (SchoolSpecialistUserModel model in listPrincipal)
            {
                PrincipalEntity item = null;
                if (model.ID <= 0)
                {
        ...
        private bool CheckExisted2(PrincipalEntity principal, int schoolId, PostFormResponse response)
        {
            ...
            if (existedStatus == UserExistedStatus.ExistedInSchool)
            {
                // do nothing
                response.Success = true;
                response.Message = "More than one school specialist with the same first name, last name, and primary email already exists in the same school: "
            ...
            else if (existedStatus == UserExistedStatus.UserExisted)
            {
                if (existedUserRole == Role.School_Specialist)
                {
                    // do nothing
                    response.Success = true;
                    response.Message = "More than one school specialist with the same first name, last name, and primary email already exists in Cli Engage: "
                      + principal.UserInfo.FirstName + " " + principal.UserInfo.LastName + " "
                      + principal.UserInfo.PrimaryEmailAddress;
        ...
        private bool CheckExisted3(PrincipalEntity principal, UserExistedStatus existedStatus, PostFormResponse response)
        {
            if (existedStatus == UserExistedStatus.UserExisted)
            {
                ...
                response.Success = true;
                response.Message = "More than one school specialist with the same first name, last name, and primary email already exists in this list: "
