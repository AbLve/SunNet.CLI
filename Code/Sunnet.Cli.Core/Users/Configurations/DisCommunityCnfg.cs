...
{
    public class DisCommunityCnfg : EntityTypeConfiguration<CommunityUserEntity>, IEntityMapper
    {
        public DisCommunityCnfg()
        {
            ToTable("CommunityUsers");
            HasRequired(x => x.UserInfo).WithOptional(o => o.CommunityUser).Map(conf => conf.MapKey("UserId"));
